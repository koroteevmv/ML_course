...
print(f"Linear Regression-Training set score: {lr.score(X_train, y_train):.4f}")
print(f"Linear Regression-Test set score: {lr.score(X_test, y_test):.4f}")
```
...
Данный код построит 50 моделей, различающихся только параметром регуляризации. Он будет лежать в диапазоне от 100 ($10^2$) до 100 000 ($10^6$). Конкретные 
