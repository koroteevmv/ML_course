...
1. Загрузите датасет ирисы Фишера `load_iris()` из библиотеки `sklearn.datasets`. Отделите целевую переменную `class` от остальных данных.
2. Сделайте количественное описание датасета: число примеров, число признаков, статистику по признакам.
3. Сделайте hold-out разбиение данных. Для этого разделите данные на обучающую и валидационную выборки при помощи `train_test_split` из библиотеки `sklearn.model_selection` в соотношении 80-20 (для этого укажите параметр test_size=0.2) без перемешивания `shuffle=False` и выведите на экран соответствующие индексы разбиения.
...
5. Создайте объект логистической регрессии `LogisticRegression(solver='liblinear')`. Обучите модель на обучающих (трейновых) данных. Выведите значения коэффициентов модели, полученных в результате обучения. Сделайте предсказание на валидационном наборе признаков. Выведите значение метрик `accuracy` и `f1-score`.
...
7. Теперь сделайте k-блочную перекрёстную проверку модели (кросс-валидацию). Для этого сделайте разбиение данных на k одинаковых по количеству  блоков (фолдов) при помощи `KFold` из библиотеки `sklearn.model_selection` и выведите на экран соответствующие инднксы разбиения. Разбиение делайте с перемешиванием `shuffle=True` и фиксированным воспроизведением перемешивания `random_state=42`. Число фолдов возьмите равным семи `n_splits = 7`. Обучите модель логистической регрессии на каждом из фолдов. Выведите на экран массивы с соответствующими метриками. В качестве значения каждой из метрик, возьмите среднее значение.
Сравните полученные метрики с метриками, которые были при hold-out разбиении.
Теперь сделайте ту же самую перекрёстную проверку модели, используя библиотечную функцию `cross_val_score`, где в качестве значения параметра  `cv` укажите число блоков равное 7, а в качестве выводимой метрики `scoring = 'accuracy'`. Убедитесь, что получится тот же результат.
8.  Теперь сделайте k-блочную перекрёстную проверку модели (кросс-валидацию) со стратификацией. Для этого сделайте разбиение со стратификацией данных на k одинаковых по количеству блоков (фолдов) при помощи `StratifiedKFold` из библиотеки `sklearn.model_selection`. Проделайте всё тоже самое, что и в предыдущем пункте.
Делая перекрёстную проверку модели, используя библиотечную функцию `cross_val_score`, где в качестве значения параметра  `cv` укажите тип стратифицированного разбиения.
9. Теперь сделайте перекрёстную проверку, изпользуя leave-one-out разбиение. Это разбиение, при котором в качестве обучающей выборки рассматриваются  𝑛−1  пример, а в качестве валидационной  1  пример из исходных данных. Для этого сделайте разбиение при помощи `LeaveOneOut` из библиотеки `sklearn.model_selection`. Проделайте всё тоже самое, что и в предыдущем пункте.
...
![График регрессии](https://github.com/koroteevmv/ML_course/blob/2023_new/ML4.4%20cross-validation/ml44-1.png?raw=true)
